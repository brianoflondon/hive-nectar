...
from beem.exceptions import InvalidWifError, MissingKeyError
...
class Testcases(unittest.TestCase):
    ...
    def test_wallet_keys(self):
        ...
        priv_key = stm.wallet.getPrivateKeyForPublicKey(
            str(PrivateKey(self.active_private_key_of_beem4, prefix=stm.prefix).pubkey)
        )
        ...
        priv_key = stm.wallet.getPrivateKeyForPublicKey(
            str(PrivateKey(self.active_private_key_of_beem5, prefix=stm.prefix).pubkey)
        )
    ...
    def test_create_account(self):
        ...
        self.assertIn(format(key5.pubkey, core_unit), [x[0] for x in op[role]["key_auths"]])
        self.assertIn("beem1", [x[0] for x in op[role]["account_auths"]])
        role = "owner"
        self.assertIn(format(key5.pubkey, core_unit), [x[0] for x in op[role]["key_auths"]])
        self.assertIn(format(key5.pubkey, core_unit), [x[0] for x in op[role]["key_auths"]])
        self.assertIn("beem1", [x[0] for x in op[role]["account_auths"]])
        self.assertEqual(op["creator"], "beem")
    ...
    def test_approvewitness(self):
        ...
        op = tx["operations"][0][1]
        self.assertIn("beem1", op["witness"])
